...
function Hoteldetails() {
  ...
  var fetchRoomfacilities = "http://localhost:8080/facilities";

  var [facilities, setFacilities] = useState([]);

  var fetchUrl = "http://localhost:8080/hotel/details";
  ...
  async function deletefacility(e) {
    ...
    const res = await axios.delete(
      `http://localhost:8080/facility/delete/${id_}`,
      { data: { id: 5 } }
    );
    console.log(res.data);
  }

  var fetchRoomType = "http://localhost:8080/room_types";
  ...
  var renderFacilityHeader = () => {
    let header = ["facility Type", "Facility Cost"];
    ...
    return (
      <div className="table">
        <div className="table-row">
          ...
          <div className="table-data">Taj</div>
        </div>
        <div className="table-row">
          <div className="table-data">
            <AiFillPhone /> Number
          </div>
          <div className="table-data">+94 757878788</div>
        </div>
        <div className="table-row">
          <div className="table-data">
            <MdEmail /> EmailID
          </div>
          <div className="table-data">tag@gmail.com</div>
        </div>
        <div className="table-row">
          <div className="table-data">
            <MdLocationOn /> Address
          </div>
          <div className="table-data">colombo 05, gall face, taj</div>
        </div>
      </div>
    );
  };
  ...
  const roomf = (a, b) => {
    return (
      <div>
        <div className="table-row">
          <div className="table-data">wifi</div>
          <div className="table-data">free</div>
        </div>

        <div className="table-row">
          <div className="table-data">Breakfase</div>
          <div className="table-data">$ 0.9</div>
        </div>
      </div>
    );
  };
  const roomty = (a, b) => {
    return (
      <div className="table-row">
        <div className="table-data">A/c</div>
        <div className="table-data">$ 20</div>
      </div>
    );
  };
  return (
